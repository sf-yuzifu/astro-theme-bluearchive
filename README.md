...
bangumi:
  ...
  # 集成配置
  integration:
    source: "bili"  # 'bili' | 'bgmv0' - 数据源选择
    vmid: "你的用户ID"  # 用户 ID（必填，Bilibili UID 或 BGMTV UID）
    title: "追番列表"  # 页面标题
    quote: "生命不息，追番不止！"  # 页面引言
    coverMirror: ""  # 封面图片代理地址，用于解决防盗链
    category: [1, 2]  # 分类：1=番剧, 2=影视（仅 bili 源）
    lazyload: true  # 启用图片懒加载
    devMode: true  # 开发模式，开发环境下自动获取数据
    refreshEndpoint: "/api/bangumi/refresh"  # 刷新 API 端点路径，设为 false 禁用
    showMyComment: false  # 显示个人评论（仅 bgmv0 源）
    localDataPath: "./bangumi-local.json"  # 本地番剧数据文件路径（相对项目根目录）
    extraOrder: 0  # 本地数据显示顺序：1=优先显示，其他=默认顺序
  # 组件配置
  component:
